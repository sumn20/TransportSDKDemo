...
bool NDKCamera::MatchCaptureSizeRequest(ANativeWindow *display,
    ...
    LOGW("DisplayDimension %dx%d", disp.org_width(), disp.org_height());
    ...
    if (foundIt) {
        LOGW("find  compatible camera resolution, taking %dx%d", foundRes.org_width(),
             foundRes.org_height());
...
void NDKCamera::setSateListener(StateListener* state) {
    pStateListener = state;
