...
/**
 * ImageReader listener: called by AImageReader for every pFrame captured
...
ImageReader::ImageReader(ImageFormat *res, enum AIMAGE_FORMATS format)
    ...
    pVideoEncode = new FFmpegVideoEncode();
}
...
void ImageReader::PresentImage90(ANativeWindow_Buffer *buf, AImage *image) {
    ...
    if (pVideoEncode) {
        ...
        yuvData->y_buffer =yPixel;
        yuvData->u_buffer =uPixel;
        yuvData->v_buffer =vPixel;
        pVideoEncode->encode(yuvData);
        /*  std::thread encodeThread(&ImageReader::encode, this,yuvData);
           encodeThread.detach();*/
    }
    int32_t uvPixelStride;
    AImage_getPlanePixelStride(image, 1, &uvPixelStride);
...
void ImageReader::setVideoDataListener(VideoDataListener *videoDataListener) {
    if (pVideoEncode) {
        pVideoEncode->setVideoDataListener(videoDataListener);
    }

}

void ImageReader::encode(YuvData *yuvData) {
    pVideoEncode->encode(yuvData);
}
