...
void OnSessionClosed(void *ctx, ACameraCaptureSession *ses) {
    ...
    std::thread callListenThread(&NDKCamera::CallListener,
                                 static_cast<NDKCamera *>(ctx),CaptureSessionState::CLOSED);
    callListenThread.detach();
}
...
void OnSessionReady(void *ctx, ACameraCaptureSession *ses) {
    ...
    std::thread callListenThread(&NDKCamera::CallListener,
                                 static_cast<NDKCamera *>(ctx),CaptureSessionState::READY);
    callListenThread.detach();
}
...
void OnSessionActive(void *ctx, ACameraCaptureSession *ses) {
    ...
    std::thread callListenThread(&NDKCamera::CallListener,
                                    static_cast<NDKCamera *>(ctx),CaptureSessionState::ACTIVE);
    callListenThread.detach();
...
void NDKCamera::CallListener(CaptureSessionState state) {
    if (pStateListener) {
        pStateListener->actionPerformed(state);
    }
